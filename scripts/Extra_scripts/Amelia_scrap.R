...
artio_full <- read.csv(here("sleepy_artiodactyla_full.csv")) 
artio_full <- artio_full[!is.na(artio_full$max_crep), ] #317 species (82 cetaceans, 235 non cetaceans)
artio_full <- artio_full %>% select(Species_name, Order, Family, max_crep)
new_mammals <- new_mammals %>% select(Species_name, Order, Family, Bennie_activity_pattern)
colnames(new_mammals) <- colnames(artio_full)
...
diel.plot <- diel.plot + geom_tile(data = diel.plot$data[1:length(trpy_n$tip.label),], aes(x=x, y=y, fill = max_crep), inherit.aes = FALSE, colour = "transparent", width = 6) + scale_fill_manual(values = custom.colours)
geom_cladelab(barsize = 1.5, barcolor = "grey50", node = node_labels_left$node_number, label = node_labels_left$clade_name, hjust = 1, offset =3, offset.text = 4, fontsize = 4) + 
  geom_cladelab(barsize = 1.5, barcolor = "grey50", node = node_labels_right$node_number, label = node_labels_right$clade_name, offset = 3, offset.text = 4, fontsize= 4) + 
  ...
  geom_cladelabel(node = 4877, label = "Artiodactyla", offset.text = 80, vjust = 1, barsize = 2, fontsize = 4)
diel.plot
